...
function main() {
    ...
    for (let i = 0; i < 15; i++) {
        points.push(new THREE.Vector3(
            Math.sin(i * 0.6) * 40 + (Math.random() - 0.5) * 15,
            Math.cos(i * 0.4) * 30 + (Math.random() - 0.5) * 15,
            Math.sin(i * 0.2) * 40 + (Math.random() - 0.5) * 15
        ));
    }
    ...
    const tubeGeometry = new THREE.TubeGeometry(curve, 256, 2, 8, true);
